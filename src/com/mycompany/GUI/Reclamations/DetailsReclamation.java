...
public class DetailsReclamation {
   ...
   if(User.getActifUser() !=null && User.getActifUser().getRoles().equalsIgnoreCase("[ROLE_USER]")){
        ...
        tb.addCommandToOverflowMenu("Back", null, (evt3) -> {
         ...
         details.getToolbar().addCommandToOverflowMenu("LogOut", null, new ActionListener() {
                                @Override
...
//        details.add(createForFont(String.valueOf(selRecl.getIdReclamation()), Font.STYLE_BOLD, Font.SIZE_LARGE, 0xff7800));
        ...
        Container objet6 = new Container(BoxLayout.y());
        objet6.getAllStyles().setMarginTop(30);
        Button traiter = new Button("Répondre");
        traiter.getStyle().setBgColor(0x28A745);
        traiter.getStyle().setBgTransparency(255);
        Button rejeter = new Button("Rejeter");
        rejeter.getStyle().setBgColor(0xFF0000);
        rejeter.getStyle().setBgTransparency(255);
        objet6.addAll(traiter, rejeter);
        tout.add(objet6);
        rejeter.addActionListener((evt) -> {
            if(Dialog.show("Confirmation", "Voulez-vous vraiment rejeter cette réclamation ?", "Non", "OUI")) {
                System.out.println("Nooon");
            }
            else{
                System.out.println("OK");
                objet6.removeComponent(rejeter);
                objet6.removeComponent(traiter);
                objet6.setLayout(new FlowLayout(Component.CENTER));
                objet6.getStyle().setBgColor(0xFF0000);
                objet6.getStyle().setBgTransparency(255);
                objet6.add(createForFont("Réclamation rejétée", Font.STYLE_BOLD, Font.SIZE_LARGE, 0xFFFFFF));
                ServiceReclamations ser = new ServiceReclamations();
                selRecl.setSuiviReclamation("Rejétée");
                selRecl.setDateReponseReclamation(new Date());
                ser.updateReclamation(selRecl);
            }            
        });        
        traiter.addActionListener((evt) -> {
            System.out.println("Répondre");
            ReponseReclamation r = new ReponseReclamation();
            r.getFormulaire().show();
        });
